...
#   Intializing the model
    ...
    ema_model = models.__dict__[args.model](
        args, nograd=True, data=None).cuda()
...
def train(train_loader, model, ema_model, optimizer, epoch):
    ...
    for i, ((input, ema_input), target) in enumerate(train_loader):
                ...
                if args.sntg:  # implementation of SNTG loss
                    ...
                    consistency_loss = consistency_weight * \
                        consistency_criterion(
                            model_out, ema_logit) / minibatch_size
                    sntg_loss = (0.001/2) * sntg_loss * \
                        consistency_weight  # best 0.001/2
                else:
                    consistency_weight = get_current_consistency_weight(epoch)
                    consistency_loss = consistency_weight * \
                        consistency_criterion(
        ...
        if i % 20 == 19:    # print every 20 mini-batches
            print(
                f'[Epoch: {epoch + 1}, Iteration: {i + 1}] loss: {running_loss / 20}')
            running_loss = 0.0
...
if __name__ == '__main__':
    ...
    args.supervised_mode = True
    args.checkpoint_epochs = 10
