...
from pytorch.mean_teacher import architectures, datasets, data, losses, ramps, cli
...
class SOM(nn.Module):
    ...
    def forward(self, x, y, it):
        ...
        delta = torch.mul(learning_rate_multiplier.to(device), (torch.stack(
            [x.squeeze() for i in range(self.m * self.n)]).to(device) - self.weights))
...
class model_params:
    def __init__(self):
        self.arch_name = 'cifar_shakeshake26'
        self.lr = 0.1
        self.momentum = 0.9
        self.nesterov = False
...
ds_name = 'cifar10'
checkpoint_path = "checkpoint.300.ckpt"
...
EPS = 100
for n in [6, 7, 8, 9, 10, 15]:
    # n = 8

    som = SOM(n, n, 384, EPS, {}).to(device)
    som.train()
    ...
    for ep in range(EPS):
            ...
            for i, x_c in enumerate(loaded_data, 0):
                if i % 5000 == 0:
                    ...
                    with open("logs/fv-logs.log", "a") as a:
                        a.write(
                            f"{i} / {len(loaded_data)}, n = {n}, time: {tm() - t}\n")
                input, label = x_c
                input = input.view(-1)
                som(input, label, ep)
        ...
        if ep % 5 == 4:
            ...
            show_som_stats(som.all_quant_err, som.all_winner, som.all_entr,
                           som.all_dists, f"figsf/fv-{n}n-{ep}ep-stat.png")
            # print(som.d)
    ...
    training = {"quant_err": [t.tolist() for t in som.all_quant_err], "wd": som.all_winner, "entropy": [t.tolist() for t in som.all_entr], "d": som.d}
